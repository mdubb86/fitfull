...
describe('htmlToTokens', () => {
    ...
    test('plain text produces word+space tokens', () => {
        const tokens = htmlToTokens(styled('Hello World'));
    ...
    test('<b> produces bold weight', () => {
        const tokens = htmlToTokens(styled('<b>Hello</b> World'));
    ...
    test('<strong> produces bold weight', () => {
        const tokens = htmlToTokens(styled('<strong>Hi</strong>'));
        assert.strictEqual(tokens[0].weight, 'bold');
    });
    ...
    test('<i> produces italic weight', () => {
        const tokens = htmlToTokens(styled('<i>Hello</i>'));
        assert.strictEqual(tokens[0].weight, 'italic');
    });

    test('<em> produces italic weight', () => {
        const tokens = htmlToTokens(styled('<em>Hello</em>'));
        assert.strictEqual(tokens[0].weight, 'italic');
    });
    ...
    test('nested <b><i> produces bolditalic', () => {
        const tokens = htmlToTokens(styled('<b><i>Hello</i></b>'));
        assert.strictEqual(tokens[0].weight, 'bolditalic');
    });

    test('nested <i><b> produces bolditalic', () => {
        const tokens = htmlToTokens(styled('<i><b>Hello</b></i>'));
        assert.strictEqual(tokens[0].weight, 'bolditalic');
    });
    ...
    test('inline font-size in px', () => {
        const tokens = htmlToTokens(styled('<span style="font-size: 24px">Big</span>'));
        assert.strictEqual(tokens[0].size, 24);
    });

    test('inline font-size in pt', () => {
        const tokens = htmlToTokens(styled('<span style="font-size: 12pt">Text</span>'));
        assert.ok(Math.abs(tokens[0].size - 16) < 0.01);
    });

    test('inline font-size in em', () => {
        const tokens = htmlToTokens(styled('<span style="font-size: 2em">Text</span>'));
        assert.strictEqual(tokens[0].size, 24);
    });

    test('inline font-size in percent', () => {
        const tokens = htmlToTokens(styled('<span style="font-size: 200%">Text</span>'));
        assert.strictEqual(tokens[0].size, 24);
    });
    ...
    test('font-weight: bold via CSS', () => {
        const tokens = htmlToTokens(styled('<span style="font-weight: bold">Text</span>'));
        assert.strictEqual(tokens[0].weight, 'bold');
    });

    test('font-weight: 700 via CSS', () => {
        const tokens = htmlToTokens(styled('<span style="font-weight: 700">Text</span>'));
        assert.strictEqual(tokens[0].weight, 'bold');
    });

    test('font-weight: 400 stays regular', () => {
        const tokens = htmlToTokens(styled('<span style="font-weight: 400">Text</span>'));
    ...
    test('font-family is normalized', () => {
        const tokens = htmlToTokens(styled('<span style="font-family: Open Sans">Text</span>'));
        assert.strictEqual(tokens[0].font, 'open-sans');
    });

    test('font-family with quotes and fallbacks', () => {
        const tokens = htmlToTokens(styled('<span style="font-family: \'Roboto Mono\', monospace">Text</span>'));
    ...
    test('font-style: italic via CSS', () => {
        const tokens = htmlToTokens(styled('<span style="font-style: italic">Text</span>'));
        assert.strictEqual(tokens[0].weight, 'italic');
    });
    ...
    test('whitespace between tags collapses', () => {
        const tokens = htmlToTokens(styled('<b>Hello</b>  <i>World</i>'));
        assert.strictEqual(tokens.length, 3);
        assert.strictEqual(tokens[1].text, ' ');
    });
    ...
    test('br tag produces newline token', () => {
        const tokens = htmlToTokens(styled('Hello<br>World'));
        assert.strictEqual(tokens.length, 3);
        assert.strictEqual(tokens[0].text, 'Hello');
        assert.strictEqual(tokens[1].text, '\n');
        assert.strictEqual(tokens[2].text, 'World');
    ...
    test('HTML comments are ignored', () => {
        const tokens = htmlToTokens(styled('Hello <!-- comment --> World'));
        assert.strictEqual(tokens.length, 3);
        assert.strictEqual(tokens[0].text, 'Hello');
    });

    test('empty elements produce no tokens', () => {
        const tokens = htmlToTokens(styled('<b></b>Hello'));
        assert.strictEqual(tokens.length, 1);
        assert.strictEqual(tokens[0].text, 'Hello');
    });

    test('deeply nested structure', () => {
        const tokens = htmlToTokens(styled('<div><p><b><i>Deep</i></b></p></div>'));
        assert.strictEqual(tokens[0].weight, 'bolditalic');
    ...
    test('body styling sets defaults', () => {
        const tokens = htmlToTokens(styled('Hello', { font: 'arial', size: 16, weight: 'bold' }));
        assert.strictEqual(tokens[0].weight, 'bold');
    ...
    test('missing font-family throws', () => {
        assert.throws(
            () => htmlToTokens('<body style="font-size: 12px">Hello</body>'),
            /font-family/
        );
    });
    ...
    test('missing font-size throws', () => {
        assert.throws(
            () => htmlToTokens('<body style="font-family: inter">Hello</body>'),
            /font-size/
        );
    });
    ...
    test('cross-style word produces separate adjacent tokens', () => {
        const tokens = htmlToTokens(styled('<b>He</b>llo'));
